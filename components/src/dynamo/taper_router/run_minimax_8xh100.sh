...
#
# Bring up the MiniMax-M2 A/B/C stack on one 8xH100 node, adding the TAPER
# arm alongside thunderagent_router's existing ta|kv choice. Usage:
#   ./run_minimax_8xh100.sh ta
#   ./run_minimax_8xh100.sh kv
#   ./run_minimax_8xh100.sh taper
#
# Structurally identical to thunderagent_router/run_minimax_8xh100.sh (same
# model, same ports, same worker launch) so a Harbor/Pi run only has to swap
# --job-name/$ARM between the two scripts -- everything else in the walkthrough
# (README.md "Harbor/Pi A/B walkthrough") stays the same.
set -euo pipefail
...
POLICY="${1:-}"
if [[ "$POLICY" != "ta" && "$POLICY" != "kv" && "$POLICY" != "taper" ]]; then
    echo "usage: $0 ta|kv|taper" >&2
    exit 2
fi
...
if [[ "$POLICY" == "ta" || "$POLICY" == "taper" ]]; then
